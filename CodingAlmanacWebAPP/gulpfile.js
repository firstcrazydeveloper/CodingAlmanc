...
gulp.task('scriptsControllers', ['clean'], function () {
    ...
    return gulp.src(['./App/Dashboard/*.js', './App/Share/*.js'])
...
gulp.task('styles', ['clean'], function () {
    // Minify and copy all JavaScript (except vendor scripts) 
    // with sourcemaps all the way down 
    return gulp.src('./Resources/Site.css')
...
gulp.task('scriptsServices', ['clean'], function () {
    ...
    gulp.src('./App/CodingAlmanac.js')
         .pipe(angularFilesort())
        .pipe(concat('CodingAlmanac.js'))
    .pipe(uglify())
      .pipe(rename('CodingAlmanac.min.js'))
      .pipe(rev())
      .pipe(gulp.dest('./dest/'))
    ...
    // Minify and copy all JavaScript (except vendor scripts) 
    // with sourcemaps all the way down 
    return gulp.src(['Common/Constants.js', "Common/bootstrap/CodingAlmanac.bootstrap.dialog.js", 'Common/Services/*.js'])
      .pipe(angularFilesort())
...
gulp.task('index', ['scriptsControllers', 'scriptsDirectives', 'scriptsServices', 'styles'], function () {
    ...
    .pipe(inject(gulp.src(['Scripts/angular.min.js', 'Scripts/angular-route.js', 'scripts/angular-sanitize.js', 'Scripts/ui-bootstrap-tpls-0.10.0.min.js', 'Scripts/angular-animate.js', 'Scripts/angular-resource.js', 'scripts/toastr.js', 'scripts/moment.js', 'scripts/bootstrap.js', 'scripts/moment.js', 'scripts/spin.js', 'scripts/ng-grid.js'], { read: false }), { name: 'library' }))
    .pipe(inject(gulp.src('Common/Bundles/gulp/*.js', { read: false }), { relative: false }))
    .pipe(inject(gulp.src(['./dest/*.js', './AppConfig.js'], { read: false }), { name: 'CodingAlmanac' }))
    .pipe(gulp.dest('./'))
